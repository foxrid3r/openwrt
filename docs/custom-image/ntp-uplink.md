...
- `defaultroute '1'` lets DHCP install a route through its dynamically assigned gateway. The firewall policy, rather than the absence of a route, restricts how the uplink can be used.

## NTP Destinations

The router uses these literal NIST server addresses:
...
Using literal IP addresses avoids needing DNS resolution for the NTP service. The firewall rule limits outbound NTP traffic to these addresses; separate `/32` routes are unnecessary because the DHCP default route supplies the upstream gateway.
...
The DHCP default route should use the `ntp_uplink` interface and its assigned upstream gateway. For example, verify the selected path to a configured server:
...
2. Enable **Use DNS servers advertised by peer**.
3. Go to **Network → Firewall → General Settings** and edit the `ntp_uplink` zone.
4. Temporarily change **Output** from **reject** to **accept**.
5. Select **Save & Apply**, perform the package maintenance, and then restore both settings:
...
Confirm `ip route get 129.6.15.28` and `ip route get 132.163.96.1` select the DHCP gateway through `ntp_uplink`.
...
- The NTP allow rule is limited to UDP/123 and the configured server IPs.
- No additional firewall rules allow external traffic.
